...
import numpy as np
from details.Detector import MLD
...
class Detector3:
        ...
        for signy in x_bounds.keys():
            for height in range(h_bounds[signy] + 1):
                ...
                for signx in x_bounds[signy][height].keys():
                    w_bounds[signy][height][signx] = min(
                        (x_bounds[signy][height][signx] / self.x_div), (x_bounds[signy][height + 1][signx] / self.x_div))
    ...
    def detect(self, point: Point):
        ...
        if h <= self.h_bounds[sign_y] and w <= self.w_bounds[sign_y][h][sign_x]:
            if abs(w - h) % 2 == 0:
                cand_1 = Point(sign_x * (w + 1) * self.x_div,
                               sign_y * h * self.y_div)
                cand_2 = Point(sign_x * w * self.x_div,
                               sign_y * (h + 1) * self.y_div)
            else:
                cand_1 = Point(sign_x * w * self.x_div,
                               sign_y * h * self.y_div)
                cand_2 = Point(sign_x * (w + 1) * self.x_div,
                               sign_y * (h + 1) * self.y_div)
